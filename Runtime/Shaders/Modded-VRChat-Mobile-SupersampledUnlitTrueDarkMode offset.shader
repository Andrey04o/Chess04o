...
Shader "Modded/Mobile/Worlds/Supersampled True DarkMode Offset"
{
    Properties
    {
        [PerRendererData] _MainTex ("Sprite Texture", 2D) = "white" {}
        _Color ("Tint", Color) = (1,1,1,1)
        _Color2 ("Tint dark", Color) = (0,0,0,1)
    ...
    SubShader
    {
        ...
        CGPROGRAM
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                // color change
                fixed maxColor = color.x;
                fixed minColor = color.x;
                
                if (color.y > maxColor) maxColor = color.y;
                if (color.z > maxColor) maxColor = color.z;
                if (minColor > color.y) minColor = color.y;
                if (minColor > color.z) minColor = color.z;
                fixed difColor = 1.0 - maxColor - minColor;
                //fixed difColor = 1.0 - minColor;
                color.x = color.x + difColor;
                color.y = color.y + difColor;
                color.z = color.z + difColor;
                //fixed4 colorW = color * _Color;
                //fixed4 colorB = (1.0 - color) * _Color2;
                //color.x = colorW.x + colorB.x;
                //color.y = colorW.y + colorB.y;
                //color.z = colorW.z + colorB.z;
